...
const uriToBase64 = async (uri: string): Promise<string> => {
    try {
        ...
        log('Error converting audio to base64:', error);
        throw error;
    }
};
...
export const checkPronunciation = async (
    ...
    try {
        ...
        return await fetchWithLogging('https://sesefi-lexi-reading-pronunciation-api.hf.space/pronunciation', {
            method: 'POST',
...
export const analyzeReadingAssessment = async (
    ...
    try {
        const base64Audio = await uriToBase64(audioUri);
        const result = await fetchWithLogging('https://sesefi-lexi-reading-pronunciation-api.hf.space/reading', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
